...
该值表示从上次查询到现在为止该节点由于`流控制`而暂停的时间比例.
...
监控`流控制`和接收队列可以告诉你数据库如何应付接收的`write-sets`,同样你也可以通过监控发送队列来查看节点发送情况.这个值同样表示从上次执行查询开始到现在为止该期间的一些情况.
