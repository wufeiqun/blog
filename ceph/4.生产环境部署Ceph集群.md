...
新建集群后其实只是部署了单台ceph, 后续需要通过添加节点的方式扩容集群, 我发现我添加了host后, ceph自动帮我部署了monitor节点, 然而我并没有主动添加monitor. 我后来查询后了解到ceph默认的集群会配置5个monitor, 新加入host节点的时候, ceph发现monitor不足5个, 就会自动在该节点上安装monitor, 直到满足5个为止, 检测方式:

```
# PLACEMENT这一列是期望数量, 可以看到mon节点是5个, 带星号的是每个host都部署.
[root@server131 ~]# ceph orch ls
NAME           PORTS        RUNNING  REFRESHED  AGE  PLACEMENT
alertmanager   ?:9093,9094      1/1  4m ago     4h   count:1
ceph-exporter                   4/4  7m ago     4h   *
crash                           4/4  7m ago     4h   *
grafana        ?:3000           1/1  4m ago     4h   count:1
mgr                             2/2  7m ago     4h   count:2
mon                             4/5  7m ago     31m  count:5
node-exporter  ?:9100           4/4  7m ago     4h   *
prometheus     ?:9095           1/1  4m ago     4h   count:1
```

ceph有很多种服务编排的方式, 也就是哪些进程要在哪些节点上启动. 对于生产环境我自己习惯固定的方式, 比如Monitor节点, 我要指定具体的主机名:

```
# 参考文档 https://docs.ceph.com/en/latest/cephadm/services/#orchestrator-cli-placement-spec
ceph orch apply prometheus --placement="server131 server132 server133"
或者
ceph orch apply prometheus --placement="3 server131 server132 server133"
```
