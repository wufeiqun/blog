...
/data     *(rw,sync,no_all_squash,no_subtree_check)
...
```
/data: 共享目录位置。
*: 客户端 IP 范围，* 代表所有，即没有限制, 也可以写CIDR网段更加安全。
rw: 权限设置，读写权限。
sync: 数据写入同步到磁盘，更安全。
no_all_squash: 所有用户都不映射, 为了安全默认会映射成nobody, 不映射就是客户端是什么用户, 映射到服务端就是什么用户, 这样配置限制最小。
...
```
mount -t nfs  <服务器IP>:/data  /data/share
```
