...
[root@localhost ~]# vgs
  VG           #PV #LV #SN Attr   VSize   VFree
  rockylinuxvg   1   1   0 wz--n- <28.00g 10.00g
...
[root@localhost ~]# lvdisplay
  --- Logical volume ---
  LV Path                /dev/rockylinuxvg/root
  LV Name                root
  VG Name                rockylinuxvg
  LV UUID                2j9FtB-O3be-cjcj-xThp-ZWYG-wuJD-iyC8y4
  LV Write Access        read/write
  LV Creation host, time localhost.localdomain, 2025-11-07 10:00:43 +0800
  LV Status              available
  # open                 1
  LV Size                <18.00 GiB
  Current LE             4607
...
```
# 按剩余空间比例扩容
lvextend -r -l +100%FREE /dev/rockylinuxvg/root

* -r = 自动 resize filesystem（扩文件系统，无需再用 resize2fs）
* -l +100%FREE = 使用当前卷组(VG)中所有剩余空间
* /dev/rockylinuxvg/root = 要扩的逻辑卷路径（可通过 lvs 或 df -h 查看）

# 扩容具体的空间
lvextend -r -L +5G /dev/rockylinuxvg/root
```
...
tmpfs                          341M  6.1M  335M   2% /run
efivarfs                       256K   32K  225K  13% /sys/firmware/efi/efivars
/dev/mapper/rockylinuxvg-root   18G  1.3G   16G   8% /
/dev/nvme0n1p2                 974M  171M  736M  19% /boot
/dev/nvme0n1p1                1022M  7.3M 1015M   1% /boot/efi
tmpfs                          171M     0  171M   0% /run/user/0
[root@localhost ~]# lvextend -r -l +100%FREE /dev/rockylinuxvg/root
  File system ext4 found on rockylinuxvg/root mounted at /.
  Size of logical volume rockylinuxvg/root changed from <18.00 GiB (4607 extents) to <28.00 GiB (7167 extents).
  Extending file system ext4 to <28.00 GiB (30060576768 bytes) on rockylinuxvg/root...
resize2fs /dev/rockylinuxvg/root
resize2fs 1.46.5 (30-Dec-2021)
Filesystem at /dev/rockylinuxvg/root is mounted on /; on-line resizing required
old_desc_blocks = 3, new_desc_blocks = 4
The filesystem on /dev/rockylinuxvg/root is now 7339008 (4k) blocks long.

resize2fs done
  Extended file system ext4 on rockylinuxvg/root.
  Logical volume rockylinuxvg/root successfully resized.
[root@localhost ~]# df -h
Filesystem                     Size  Used Avail Use% Mounted on
devtmpfs                       4.0M     0  4.0M   0% /dev
tmpfs                          851M     0  851M   0% /dev/shm
tmpfs                          341M  6.1M  335M   2% /run
efivarfs                       256K   32K  225K  13% /sys/firmware/efi/efivars
/dev/mapper/rockylinuxvg-root   28G  1.3G   25G   5% /
/dev/nvme0n1p2                 974M  171M  736M  19% /boot
/dev/nvme0n1p1                1022M  7.3M 1015M   1% /boot/efi
tmpfs                          171M     0  171M   0% /run/user/0
```
...
3. LVM可能出现某一个磁盘坏了导致所有数据丢失的问题, 也可能出现因为不通磁盘的IO情况不同而影响读写
